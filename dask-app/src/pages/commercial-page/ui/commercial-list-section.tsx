...
import {
  ...
  isApprovedProposal,
  isOpenBillingStatus,
  type FilteredCommercialMetrics
...
interface OpportunityActionHandlers {
  ...
  onOpenCustomerFromWorkItem: (task: Task) => void;
...
function resolvePrimaryAction(row: OpportunityRow, handlers: OpportunityActionHandlers, signalTypeIds: string[]): OpportunityAction {
  ...
  if (!row.customer) {
    return {
      ...
      icon: "user",
      onClick: () => handlers.onOpenCustomerFromWorkItem(row.task)
    };
  }
  ...
  if (!row.contract) {
    return {
      ...
      description: "Formalizar a proposta ja registrada para este workItem.",
      icon: "documentation",
      onClick: handlers.onOpenDocs
    };
  }
...
function CommandMetric({
  ...
  return (
    <article className={cn("commercial-command-metric", `commercial-command-metric--${tone}`)}>
      <span className="commercial-command-metric__icon" aria-hidden="true">
        <AppIcon name={icon} size={17} />
      </span>
      <span className="commercial-command-metric__body">
        <span>{label}</span>
        <strong>{value}</strong>
        <small>{detail}</small>
      </span>
...
export function CommercialListSection({
  filteredTasks,
  ...
  filteredTasks: Task[];
  filteredWorkItemMetrics: FilteredCommercialMetrics;
  search: string;
    ...
    () =>
      filteredTasks.map((task) => {
        ...
        return {
          ...
          workItemSubtitle: resolveCatalogLabel(getTextField(task, "interest")) || task.text || "Escopo comercial a qualificar",
  ...
  return (
    <ResourceSection variant="plain" className="commercial-opportunities-shell">
      <header className="commercial-command-hero">
        <div className="commercial-command-hero__copy">
          <span className="commercial-page__eyebrow">Pipeline comercial</span>
          <h2>Comercial comerciais</h2>
          <p>
            Central de oportunidades conectando workItem, cliente, proposta, contrato, cobranca e execucao em um fluxo operacional.
          </p>
        </div>
        <div className="commercial-command-hero__value">
          <span>Valor em aberto</span>
          <strong>{formatMoneyCompact(filteredWorkItemMetrics.openValue)}</strong>
          <small>Pipeline estimado {formatMoneyCompact(filteredWorkItemMetrics.totalValue)}</small>
        </div>
      </header>

      <div className="commercial-command-metrics">
        <CommandMetric
          label="Ativos"
          value={filteredWorkItemMetrics.activeCount}
        ...
        <CommandMetric
          label="Sem cliente"
          value={filteredWorkItemMetrics.unlinkedCount}
        ...
        <CommandMetric
          label="Com proposta"
          value={filteredWorkItemMetrics.proposalCount}
        ...
        <CommandMetric
          label="Cobranças"
          value={filteredWorkItemMetrics.billingCount}
          detail={`${formatMoneyCompact(filteredWorkItemMetrics.openValue)} em aberto`}
          icon="wallet"
          tone="green"
        />
        <CommandMetric
          label="Ticket medio"
          value={formatMoneyCompact(filteredWorkItemMetrics.avgValue)}
        ...
        <CommandMetric
          ...
          value={filteredWorkItemMetrics.nextStepsCount}
      ...
      <div className="commercial-opportunities-toolbar shared-surface-panel">
        <FormField label="Buscar oportunidades" className="commercial-search-field">
          <span className="commercial-search-control">
        ...
        <div className="commercial-opportunities-toolbar__meta">
          <span>
            {totalCount ?? filteredTasks.length} workItem{(totalCount ?? filteredTasks.length) !== 1 ? "s" : ""}
            {search ? ` encontrado${filteredTasks.length !== 1 ? "s" : ""}` : " no total"}
          </span>
          <strong>{formatMoneyCompact(filteredWorkItemMetrics.totalValue)} em pipeline</strong>
        </div>
      </div>

      {rows.length === 0 ? (
        <EmptyState
          className="commercial-opportunities-empty"
          variant="card"
          icon={<AppIcon name="briefcase" size={22} />}
          title={search ? "Nenhuma oportunidade neste filtro" : "Nenhum workItem comercial encontrado"}
          description={search ? "Ajuste a busca para voltar ao radar comercial." : "Quando commercial comerciais entrarem no Dask, eles aparecem aqui como oportunidades operacionais."}
        />
      ) : (
          ...
          {rows.map((row) => {
            ...
            return (
              <article
                ...
                role="listitem"
                style={{ "--opportunity-color": row.stageColor ?? "var(--commercial-accent)" } as CSSProperties}
              >
                <section className="opportunity-card__workItem" aria-label="WorkItem">
                ...
                <section className="opportunity-card__pipeline" aria-label="Esteira">
                  <div className="opportunity-card__pipeline-head">
                    ...
                    <span className="opportunity-current-status">
                      <span style={{ background: row.stageColor ?? "var(--commercial-accent)" }} />
                      {row.statusLabel}
                    </span>
                  </div>
