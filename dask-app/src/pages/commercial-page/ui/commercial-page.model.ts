...
export type ModalMode =
  ...
  | "signal"
  | "customer"
  | "customer-from-workItem"
  | "link-customer"
  | "customer-detail"
  | "billing-justification"
  | "type-transformation";

export type CommercialWorkItemFormState = {
  customerId: string;
  contactName: string;
  contactEmail: string;
  contactPhone: string;
...
export function emptyCommercialWorkItemForm(): CommercialWorkItemFormState {
  return {
    customerId: "",
    contactName: "",
    contactEmail: "",
...
export function buildCustomerInputFromWorkItem(task: Task): CreateCustomerInput {
...
export function buildPipelineMetrics(
  ...
  metadata: BoardCommercialOperationalMetadata | null
) {
  ...
  return {
    ...
    lostWorkItems: lostTasks.length,
    totalWorkItems: commercialTasks.length,
    linkedCount,
...
export function buildFunnelData(commercialTasks: Task[], metadata: BoardCommercialOperationalMetadata | null) {
...
export function buildFilteredCommercialMetrics(filteredTasks: Task[], metadata: BoardCommercialOperationalMetadata | null) {
  const activeStatusIds = new Set(metadata?.activeStatusIds ?? []);
...
export type FilteredCommercialMetrics = ReturnType<typeof buildFilteredCommercialMetrics>;
