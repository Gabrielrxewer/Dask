...
import { CUSTOMER_STATUS_LABELS, getTextField, type PipelineMetrics } from "./leads-page.model";
...
export function CustomersListSection({
  ...
  return (
    <ResourceSection variant="plain">
      <div className="leads-page__filters leads-page__filters--bar shared-form-grid shared-form-grid--three">
      ...
      <ResourceTable
        ...
        columns={[
          ...
          },
          {
            ...
            header: "Documento",
            width: "0.8fr",
            render: ({ customer }) => <span className="leads-muted">{customer.document ?? "-"}</span>
          },
          {
            id: "email",
            header: "E-mail",
            width: "1.1fr",
            render: ({ customer }) => customer.email ?? <span className="leads-muted">-</span>
          },
          {
            id: "phone",
            header: "Telefone",
            width: "0.8fr",
            render: ({ customer }) => customer.phone ?? <span className="leads-muted">-</span>
          },
          {
            id: "status",
            header: "Status",
            width: "0.65fr",
            render: ({ customer }) => (
              <span className={`leads-customer-status leads-customer-status--${customer.status}`}>
          ...
          },
          {
            ...
            width: "0.8fr",
            render: ({ customer }) => <span className="leads-muted">{formatDate(customer.updatedAt)}</span>
          }
        ]}
        actions={{
          header: "Acoes",
          width: "1.1fr",
          render: ({ customer }) => (
            <div className="leads-page__row-actions shared-actions-row">
              <Button size="sm" variant="outline" onClick={() => onOpenCustomerDetails(customer.id)}>
                Detalhes
              </Button>
              <Button size="sm" onClick={() => onNewLead(customer)}>
                Novo lead
              </Button>
            </div>
          )
